...
int main(int argc, char* argv[])
{
    ...
    for(unsigned int i=0;i<templateKeypoints.size();i++)
      ...
      cv::Mat output = templateImage.clone();
      if( output.channels() == 1 )
            cvtColor( output, output, COLOR_GRAY2BGR );
      for ( size_t i = 0; i < templateKeypoints.size(); i++ )
      {
            cv::KeyPoint kl = templateKeypoints[i];
            if( kl.octave == 0)
            {
              /* get a random color */
              ...
              cv::Point pt1 = Point( kl.pt);
              /* draw line */
              //line( output, pt1, pt2, Scalar( B, G, R ), 5 );
              circle(output, pt1, 3,  Scalar( B, G, R ), 1, 8, 0);
            }
