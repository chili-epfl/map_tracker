...
#ifndef _MAP_TRACKER_H
#define _MAP_TRACKER_H
...
struct DetectionInfo
{
    ...
    // Previous image (as landmark detection might run on separate threads,
...
{
public:
    Map_Tracker(const std::string& configPath);
    Map_Tracker(const std::string& calibrationFile,
                  const std::string& externalFolder,
                  const std::vector<std::string>& landmarkFiles={});
    Map_Tracker(cv::FileStorage& calibration,
