...
VideoSourceLive::VideoSourceLive(int devicenum)
{
    resized=false;
    captureDevice.open(devicenum);
...
VideoSourceFile::VideoSourceFile(std::string printfPath)
{
    resized=false;
    //char fileName[200];
    //sprintf(fileName, printfPath.c_str());
    std::cout<< printfPath <<std::endl;
    captureDevice.open(printfPath);
...
void VideoSourceFile::grabNewFrame()
{
    captureDevice>>img;
    if(resized)resizeImage();
}
